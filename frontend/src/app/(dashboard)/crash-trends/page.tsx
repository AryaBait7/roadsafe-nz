...
export default async function CrashTrendsPage({
  ...
  const isEmpty = trends.data.every((p) => p.totalCrashes === 0);
      ...
      <div className="space-y-3 p-4">
        {isEmpty ? (
          <EmptyState
            ...
            description="Widen the year range or remove a filter to see trends."
          />
        ) : (
          <>
            <ChartPanel
                ...
                <DataTable
                  ...
                  rows={trends.data.map((point) => ({
                    ...
                    counts: severityTrends.data.map(
                      (series) =>
                        series.points.find((p) => p.year === point.year)
                          ?.value ?? 0,
                    ),
                  }))}
                  columns={[
